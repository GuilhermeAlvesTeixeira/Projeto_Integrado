...
const HomePage = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      {/* Hero Slider Section */}
      <div className={`relative bg-green-600 p-6 sm:p-8 md:p-12 lg:p-16 rounded-lg mx-4 sm:mx-6 md:mx-10 lg:mx-auto lg:max-w-6xl mt-8 mb-12 ${theme === 'dark' ? 'bg-green-800' : theme === 'high-contrast' ? 'bg-transparent border border-amber-400' : ''
        }`}>
        <div className="lg:pl-10 flex flex-col md:flex-row items-center gap-6 sm:gap-8 md:gap-12">
          <div className="flex-1 text-center md:text-left">
            <h1 className={`text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold mb-4 ${theme === 'high-contrast' ? 'text-white' : 'text-white'
              }`}>
              {heroSlides[currentSlide].title}
            </h1>
            <p className={`text-sm sm:text-base md:text-lg lg:text-xl mb-6 ${theme === 'high-contrast' ? 'text-gray-200' : 'text-white opacity-90'
              }`}>
            ...
            {currentSlide === 2 && (
              <Link
                ...
                className={`inline-block px-4 py-2 sm:px-6 sm:py-3 rounded-lg font-medium ${theme === 'light' ? 'bg-white text-green-600 hover:bg-gray-100' :
          ...
          <div className="flex-1 flex justify-center">
            <div className="relative w-full max-w-md h-[300px]">
              <Image
                ...
                className={`w-full h-full object-contain ${theme === 'high-contrast' ? 'filter contrast-125 brightness-110' : ''
                  }`}
                style={{
                  maxWidth: '100%',
                  maxHeight: '100%'
        ...
        <button
          onClick={prevSlide}
          className={`
    absolute left-2 top-1/2 -translate-y-1/2 p-2 rounded-full hover:cursor-pointer
    ${theme === 'high-contrast'
  ...
  `}
        ...
        <button
          onClick={nextSlide}
          className={`
    absolute right-2 top-1/2 -translate-y-1/2 p-2 rounded-full hover:cursor-pointer
    ${theme === 'high-contrast'
              ? 'bg-amber-400 hover:bg-amber-500 text-black'
              : theme === 'dark'
                ? 'text-white hover:bg-green-900'
  ...
  `}
      ...
      <div className="flex-1 px-4 sm:px-6 md:px-10 lg:px-0 pb-10 max-w-6xl mx-auto w-full">
        <h2 className={`text-xl sm:text-2xl font-semibold mb-8 text-center ${theme === 'light' ? 'text-gray-900' :
      ...
      {/* Footer Section */}
      <div className={`text-center p-4 mt-auto ${theme === 'light' ? 'text-gray-600' :
